%the nonsense data appears many times as "problem" - does it mean that the evaluation was not well prepared? has this hampered the conclusions? note that bad data is not design flaw - it is evaluation design flaw. Maybe to test the influence of this in the evaluation of the system you should put some sensible data and repeat one of the evaluations? there is no time for that, but you can comment on this in the discussion

%\silvia{so this means that they need more information to decide - what more do they need? is the info in the system or should the data model be extended?}

\section{Summary}
\label{evaluation-summary}

As with brainstorming, creating a user interface is not an exact science.
\silvia{i think the evaluation covers more than the UI. it also covers the process, in particular in this case because this is a totally new system}
...
\silvia{mention in the intro that this method is used}
...
\silvia{i would expect these topics to guide the discussion below. maybe you could even use these topics as headlines of paragraphs where you summarize the findings from the evaluation regarding each of these topics? i'm fine if you keep the loose discussion below, but then remove the list above because they are totally disconnected}
...
One of the testers mentioned that the system as a prototype might be used for demoing purposes. \silvia{for whom, which purpose?}
